...
from algorithms.utils.obs_preprocessing import init_obs, preprocessing
...
class FootballRunner(Runner):
    def __init__(self, config):
        ...
        self.level_path = self.all_args.init_level_dir
        self.init_level_path = self.all_args.init_level_dir
        
        if os.path.exists(self.init_level_path):
            dir_name = os.path.dirname(self.init_level_path)
            file_ext = os.path.splitext(self.init_level_path)[1]
            self.init_file_path = os.path.join(dir_name, "init_level" + file_ext)
            with open(self.init_file_path, "r") as json_file:
                self.init_level_file = json.load(json_file)
            if self.init_level_file["game_length"] != self.all_args.game_length:
                self.init_level_file["game_length"] = self.all_args.game_length
                with open(self.init_file_path, 'w') as json_file:
                    json.dump(self.init_level_file, json_file, indent=4)
            self.new_file_path = os.path.join(dir_name, self.all_args.level_dir.replace('level/', '').replace('.json', '') + file_ext)
            shutil.copy(self.init_level_path, self.new_file_path)
            with open(self.new_file_path, "r") as json_file:
                self.level_file = json.load(json_file)
        else:
            raise ValueError(f"Error: [init_level.json] does not exist.")
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if len(self.win_infos) >= 10:
                self.supervisor(
                    win_rate = np.mean(self.win_infos) >= 0.8,
                    total_num_steps = total_num_steps,
                )
                train_infos["difficulty_level"] = self.difficulty_level
                train_infos["level_stack"] = self.level_stack
                train_infos["game_win_rate"] = np.mean(self.win_infos)
                self.win_infos.clear()
    ...
    def supervisor(self, win_rate, total_num_steps):
        if win_rate >= 0.8:
            self.level_stack += 1
        ...
        result = {
            ...
            "game_length": self.all_args.game_length
        }
        with open(self.level_path, 'w') as json_file:
            ...
            for done, info in zip(dones_env, infos):
                if done:
                    ...
                    if info["score_reward"] > 0:
                        ...
                        self.win_infos.append(1)
                    else:
                        self.env_infos["win_rate"].append(0)
                        self.win_infos.append(0)
    ...
    def eval(self, total_num_steps):
        ...
        # init eval goals
        num_done = 0
        eval_goals = np.zeros(self.eval_episodes)
        eval_win_rates = np.zeros(self.eval_episodes)
        eval_steps = np.zeros(self.eval_episodes)
        step = 0
        quo = self.eval_episodes // self.n_eval_rollout_threads
        rem = self.eval_episodes % self.n_eval_rollout_threads
        ...
        while num_done < self.eval_episodes and step < self.episode_length:
            # get actions
            self.trainer.prep_rollout()
